...
def run_sensitivity(strigency, initial_samples, bo_samples, mc_samples, col):
    ...
    kern_eq = GPy.kern.RBF(input_dim=4, ARD = True) + GPy.kern.White(input_dim=4, variance=1)
    kern_bias = GPy.kern.Bias(input_dim=4)
...
for strigency in s_levels:
    ...
    run_sensitivity(strigency, 20, 20, 20000, cols[cidx])
