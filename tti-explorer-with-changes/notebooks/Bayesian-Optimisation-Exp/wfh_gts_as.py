...
def update_policy_config(go_to_school_prob, wfh_prob, asym_fac):
...
def run_tti_sim(pol_configs):
    ...
    for vals in pol_configs:
        policy_config, contacts_config = update_policy_config(vals[0], vals[1],vals[2])
...
def run_sensitivity(strigency, initial_samples, bo_samples, mc_samples, col):
    ...
    out_name = "main-effects-wfh-gts-asy-{}.pdf".format(strigency)
    ...
    out_name = "total-effects-wfh-gts-asy-{}.pdf".format(strigency)
    mlai.write_figure(filename=out_name, directory='./uq')
